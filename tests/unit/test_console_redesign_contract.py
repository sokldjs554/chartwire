...
def test_product_information_architecture_is_separated() -> None:
    ...
    for page in ("page-home", "page-session", "page-records", "page-review", "page-data", "page-engineering"):
...
def test_backend_contract_is_still_reachable() -> None:
    ...
    for needle in (
        "/v1/auth/token", "/v1/sessions?limit=", "/console/scripts.json", "/ws-ticket",
        "/ws/v1/ingest", "/ws/v1/watch", "/notes/latest", "/notes/draft",
        "/assessment", "/sign", "/revoke", "/v1/purge-jobs", "/verify-decrypt",
        "/v1/ops/dead-letters", "/metrics", "last_sent_seq", "risk.alert", "risk.ack",
    ):
...
def test_protocol_constants_and_recovery_path_remain_in_source() -> None:
    script = _script()
    for needle in (
        "0x4357", "FLAG_SIM:4", "HEADER:12", "CHUNK_MS:200", "CHUNK_BYTES:6400",
        "welcome", "ack_seq", "credit", "missing", "nack", "pong", "RESUME OK",
        "4409", "4503", "1012",
    ):
