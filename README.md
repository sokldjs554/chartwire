...
`/console` 을 열면 **제품 소개 화면**이 먼저 나옵니다. 실제 서비스 홈페이지와 같은 구성이고, 다만 채울 것이 고객사 로고나 후기가 아니라 잰 값입니다 — 「자랑할 고객사가 없어서, 잰 값을 답니다」(측정 조건과 재현 명령을 수치 옆에 같이), 「음성차팅 제품은 STT 모델 때문에 실패하지 않습니다」(부러지는 자리 네 곳), 「한 세션이 지나가는 길」 4단계, 기능 6칸, 「인증 배지는 없습니다 — 대신 테스트가 고정한 통제를 답니다」, **「무릎을 숨기지 않습니다」**(N=200 에서 처리량이 목표에 못 미친 구간), 「이 데모가 하지 않은 것」, 「그대로 돌려 보세요」, FAQ. 헤더와 히어로와 페이지 끝의 **「서비스 상담신청하기」** 는 `POST /v1/consultations` 로 실제 접수되지만 데모라 연락은 가지 않고, 폼에는 예시 값이 채워져 있어 실제 연락처를 넣지 않아도 됩니다(접수 내용은 90일 뒤 파기 — 컨테이너가 기동할 때마다 파기 명령이 돕니다). 그 옆의 **「상담 없이 데모 콘솔 열기」** 는 폼을 건너뛰고 clinician 데모 계정으로 바로 들어갑니다.
