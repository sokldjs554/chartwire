...
<http://localhost:8000> 에서 **공개 데모와 같은 화면이 같은 백엔드로** 동작합니다 — 로그인, WebSocket 기록,
근거 연결 초안, 파기 영수증까지 전부 로컬 PostgreSQL·Redis 를 실제로 씁니다. 콘솔은 부팅할 때 `GET /v1/release`
로 백엔드를 한 번 확인하고, 응답이 없을 때(백엔드 없이 정적으로 열었을 때)만 화면 흐름만 보여 주는 미리보기로
내려가며 그때는 상단에 그 사실을 띄웁니다.
