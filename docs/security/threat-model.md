...
> "완화 위치" 열의 경로가 곧 검증 지점입니다. (Phase 1 갱신: REST/파기/로그 유출 항목이 실제 파일과 테스트로 채워졌습니다.)
...
| 5 | 로그를 통한 PHI 유출 | Information disclosure | 예외 메시지나 디버그 로그에 전사 텍스트·이름·토큰·키가 찍힘 | structlog `redact_phi` 프로세서(`text, quote, name, phone, token, ticket, dek, payload` 키와 전화번호/주민번호 패턴 마스킹), crypto 예외는 키·평문을 담지 않음, 토큰 CLI 는 stdout 에만 출력 | `src/chartwire/core/logging.py`(WP-A), `src/chartwire/crypto/errors.py`, `src/chartwire/auth/cli.py`, `src/chartwire/api/middleware.py`(접근 로그에 경로·상태·소요시간만, 쿼리스트링 없음), `src/chartwire/api/app.py`(422 응답에 `input` 미포함, CryptoError 는 사유 토큰만) / `tests/integration/test_phi_logs.py`(세그먼트 복호화·환자 조회·검색·422·티켓·경보 확인 요청을 지나는 모든 stdlib 레코드 필드와 structlog 이벤트에 전사 마커·이름·전화·JWT·티켓 0건) |
...
| 7 | 파기 후 백업·WAL 잔존 | Information disclosure | 파기가 끝났는데 WAL, base backup, VACUUM 전 dead tuple 에 암호문이 남음 | **명시된 한계**: 파기 영수증은 라이브 테이블·오브젝트 스토어·Redis 만 다룸. DEK 를 먼저 crypto-shred 하므로 잔존물은 복호화 불가한 암호문이며, `verify-decrypt` 로 이를 시연 | `src/chartwire/purge/pipeline.py`(5단계 crypto-shred, `sessions_dek_guard` 트리거), `verify.py`(언래핑·샘플 복호화 실제 시도), `docs/consent-purge.md` §7, ADR-0004 / `tests/unit/test_crypto_keycache.py::test_destroyed_dek_raises_and_evicts`, `tests/integration/test_purge_pipeline.py`(`DekDestroyedError`·`DecryptError`·툼스톤 복원 거부·`verify-decrypt` 응답) |
...
| 9 | 악성 클라이언트 플러딩 | Denial of service | 크레딧을 무시하고 프레임을 쏟아붓거나 거대한 페이로드·과도한 REST 호출 | 크레딧 위반 4009 즉시 종료, 페이로드 상한 4010, REST 120/min·ws-ticket 30/min·hello 10/min 레이트리밋, 1 MB 바디 제한, `statement_timeout=5s` | `src/chartwire/ws/core.py`, `credit.py`(WP-B), `src/chartwire/redis/ratelimit.py`, `src/chartwire/api/middleware.py`(`RateLimit` 120/min·ws-ticket 30/min, `BodyLimit` 1 MB), `api/app.py` / WP-B core 테스트, `tests/integration/test_api_middleware.py::test_rate_limit_429_per_principal`, `::test_body_limit_cors_and_console`, `tests/integration/test_api_sessions.py::test_ws_ticket_roles_and_one_time_payload`(ws-ticket 버킷) |
...
| 11 | 작업 중 워커 크래시 | Tampering / DoS | 파기·초안 작업 도중 SIGKILL 로 절반만 실행된 상태가 남음 | 아웃박스 리스(`lease_s`)와 재시도, 각 파기 단계가 멱등(DELETE by id, `dek_wrapped = NULL`), `max_attempts` 초과 시 DLQ + 재실행 | `src/chartwire/outbox/poller.py`, `dlq.py`(WP-G), `src/chartwire/purge/pipeline.py`(한 트랜잭션, 단계별 멱등, 완료된 작업 재전달은 no-op), `verify.py`(실패 판정을 먼저 커밋한 뒤 예외 → DLQ) / WP-G SIGKILL 카오스 테스트, `tests/integration/test_purge_pipeline.py::test_rerun_of_a_completed_job_and_redelivery_are_no_ops`, `::test_verification_failure_is_durable_and_reaches_the_dlq` |
| 12 | owner 역할로 권한 상승 | Elevation of privilege | 앱 커넥션에서 `SET ROLE chartwire_owner` 로 RLS 를 우회 | `chartwire_app` 은 `chartwire_owner` 의 멤버가 아니며 `NOBYPASSRLS`; REST 는 `rbac.MATRIX` 로 모든 라우트 × 모든 역할을 고정하고 매트릭스에 없는 라우트는 실패(fail-closed); `role_gate` RLS 로 staff/admin 은 노트 본문을 읽을 수 없음 | `src/chartwire/db/migrations`(WP-A bootstrap), `src/chartwire/auth/rbac.py` / `tests/rls/test_rls_leak.py`(`SET ROLE` → permission denied), `tests/unit/test_auth_rbac.py`(§6.9 전 라우트 × 전 역할), `tests/integration/test_rbac_matrix.py`(실제 `create_app` 의 모든 라우트 × 5 역할 + 익명 + `service` 토큰; 매트릭스에 없는 라우트·라우트 없는 매트릭스 행은 실패) |
...
- **테넌트 간 REST 접근** (Information disclosure): 다른 테넌트의 토큰으로 환자·세션·동의·세그먼트를 조회하면 RLS 가 0행을 돌려주므로 404 이고, 블라인드 인덱스 조회는 빈 배열이다 — `tests/integration/test_api_patients_consents.py::test_tenant_b_cannot_see_tenant_a_patient`, `test_api_sessions.py::test_own_rule_list_and_tenancy`.
- **중복 제출과 재시도** (Tampering): `Idempotency-Key` 는 테넌트 범위 `SET NX` 24 h — 같은 키·같은 본문은 첫 응답을 재생하고, 같은 키·다른 본문은 `422 CW-4222` — `src/chartwire/redis/idempotency.py`, `api/middleware.py::Idempotency` / `test_api_middleware.py::test_idempotency_key_replays_the_first_response`.
- **파기 뒤 재기록** (Tampering): 파기된 세션의 세그먼트 조회는 행이 없어도 `410 CW-4100`, 파기된 환자에게는 동의를 기록할 수 없음(`409 CW-4096`), 세션 생성도 거부 — `test_api_sessions.py::test_purged_session_segments_are_410`, `test_purge_pipeline.py::test_consent_revoke_drives_patient_level_purge`.
