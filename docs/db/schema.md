...
5. **RLS 안에서는 `ILIKE`/`similarity` 가 leakproof 가 아니라 인덱스가 무시된다** (`pg_proc.proleakproof`, `docs/perf/leakproof.txt`). 정책이 걸린 테이블에 GIN 을 만들어도 `chartwire_app` 은 Bitmap+Filter 로 떨어진다. 해결: `search_segments()` SECURITY DEFINER 가 owner 로 실행되고 `segment_search` 는 FORCE 가 아닌 ENABLE 만 건다 — 이 면제는 **런타임이 소유자가 아닌** 두 역할 배포(§0.7)에서만 좁다. 단일 역할 폴백에서는 0006 이 이 테이블에도 FORCE 를 걸어 면제를 없앤다(§8).
...
8. **관리형 PG 단일 역할 폴백** (Render 등 — 환경변수가 아니라 카탈로그 조회로 감지한다): `chartwire_app` 이 없으면 `helpers.app_role_exists()` 가 False 가 되어 `helpers.grant()` 가 GRANT 를 건너뛰고, 앱은 owner 로 접속한다. `FORCE ROW LEVEL SECURITY` 덕분에 격리는 그대로다 — 그리고 이 모드에서는 0006 이 `segment_search` 에도 `FORCE` 를 건다(§5 의 owner 면제는 런타임이 소유자가 아닐 때만 안전하다). 대신 owner 면제가 사라져 검색이 RLS(느린) 경로로 떨어진다. `docs/limitations.md` 에 명시.
