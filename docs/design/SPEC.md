...
- Managed-PG fallback (`CHARTWIRE_DB_SINGLE_ROLE=1`, documented in `docs/db/schema.md`): app connects as the single available role; `FORCE ROW LEVEL SECURITY` keeps the owner subject to policies, so isolation still holds; `segment_search` (ENABLE only) loses owner exemption → search falls back to the RLS path (slower, documented).
...
8. Instruction-like text (`무시하|지시|시스템 프롬프트|ignore|instruction|진단란에|적어`) → `injection_pattern`.
...
- `render.yaml`: one web service (Docker, `chartwire serve all --embedded`: api + worker + stt-worker tasks in one process, pool 5, simulator only, `preDeployCommand: chartwire db upgrade && chartwire seed --demo --if-empty`), Render Postgres + Key Value (free), `CHARTWIRE_DB_SINGLE_ROLE=1`. `docs/limitations.md` states free-tier cold start and DB expiry; README shows the live URL only while it works.
