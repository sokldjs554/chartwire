...
`/console#/demo` 또는 첫 화면의 **3분 데모 시작**으로 진입합니다.
공용 `demo` 테넌트의 새 가상환자·동의·세션을 만들며, 기존 방문자의 세션을 재사용하지 않습니다.
약 3분은 서버 준비 이후의 체험 안내이고, 처리 완료나 타이머만으로 다음 단계가 자동 승인되지는 않습니다.

1. **실시간 상담**: 합성 대본을 전송합니다. 선택적으로 `연결 끊고 재개`를 눌러 재접속을 확인합니다. 초안이 준비돼야 다음 버튼이 활성화됩니다.
2. **초안 근거 확인**: 초안 문장을 눌러 인용 발화를 확인하고 검토 확인란을 선택합니다. 체험용 평가 문구를 확인하거나 수정합니다. 근거 미확인 문장이 있으면 서명하지 않습니다.
3. **삭제·보존 확인**: `서명 후 동의 철회·파기`를 누릅니다. 서명 → 이번 동의 철회 → 감사자 로그인 → 이번 환자의 파기 영수증 조회 → 복호화 시도 → 동일한 서명 노트 메타데이터 재조회 순서입니다.

완료 표시는 영수증 대상 일치, `verified`, 검증 항목 전체 통과, `receipt_hash_valid`, 실제 복호화 실패,
동일한 서명 노트와 보존 설정의 유지가 모두 확인될 때만 나옵니다.
전송·저장 확인 카운터는 해당 실행의 값이며, 청크 중복이나 테넌트 격리까지 이번 체험에서 검증했다는 뜻은 아닙니다.
테넌트 격리·부하·카오스 결과는 기존 별도 테스트 기록을 읽습니다.

선택한 환자·세션 또는 API 주소가 달라지면 다음 요청을 중단합니다.
철회 응답이 중간에 끊겼을 때는 같은 멱등성 키로 재시도하고, 작업 번호를 받은 뒤에는 그 작업을 다시 조회합니다.
실패한 검증을 성공으로 바꾸지 않으며, `다시 시도`와 `자유 탐색으로 전환`을 제공합니다.

**검증 범위**: `tests/unit/test_console_guided.py`는 실제 콘솔 JS와 REST 클라이언트를 상태를 가진 API 대역으로 실행합니다.
정상 흐름, 새 환자로 반복, 대상 변경 중단, 영수증·복호화·보존 실패와 재시도, 멱등성 키 유지, 근거 미확인 초안 차단을 확인합니다.
이 테스트는 PostgreSQL·Redis 통합 실행이나 브라우저 화면 검증을 대신하지 않습니다.
기존 이미지와 GIF는 안내형 데모 추가 전의 자유 탐색 콘솔 녹화입니다.

```bash
pytest tests/unit/test_console_static.py tests/unit/test_console_guided.py tests/unit/test_readme_numbers.py -q
python scripts/readme_numbers.py --check
python scripts/readme_numbers.py --readme console/index.html --check
