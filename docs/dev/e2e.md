...
- **홈**(`#/`, 로그인 전 첫 화면) — 제품 홈이다. 히어로(상담실 인라인 SVG · 상담 신청 · 데모 열기) → 히어로에 걸친 이점 4장 → 제품 화면
  한 장 → 흐름 4단계 → 보호 장치 6장 → 숫자 띠 4칸 → **콘솔에서 바로 해 보기** → 최종 CTA → FAQ → 푸터. 엔지니어링 근거는 홈에 두지 않고
  `#/tech` 로 뺐다: 측정 조건·재현 명령이 첫 화면을 덮으면 제품이 아니라 리포트로 읽힌다.
- **기술 상세**(`#/tech`, 헤더 nav 와 숫자 띠 아래에서 들어간다) — 재어 본 것 · 고비 · 안에 든 것 · 고정한 것(보호 장치 ↔ 테스트 파일) ·
  한계선 · 고른 것(채택 판정) · 직접 돌려 보기. 이 뷰에만 있는 섹션 해시(`#sec-knee` 등)로 새로고침해도 `routeFromHash` 가 뷰를 먼저 연다.
- **콘솔에서 바로 해 보기**(홈의 `#sec-console`) — **기능 검색**(검색어로 기능·대본 카드를 거르고 Enter 로 첫 결과에 들어간다), **서비스
  카드** 8장(녹음 · 라이브 · 초안 · 철회 · 복호화 · 운영 지표 · 대본 고르기 · API 문서), **주호소별 대본 카드**(`/console/scripts.json` 으로
  템플릿마다 대본과 예상 경보 수를 붙인다), 5분 투어 5단계. 카드·검색으로 들어가면 필요한 계정(④·⑤·운영 지표는 admin)으로 자동 로그인하고
...
- GIF 만들기(10 fps, 폭 940, 팔레트 40색, ≤ 8 MB — 39 초 녹화가 7.27 MB; 따뜻한 팔레트로 바꾼 뒤 그라디언트가 색을 더 먹어 1100 → 1000 → 940 으로 두 번 좁혔다). 8 MB 를 넘으면 **색을 줄이기 전에 폭을 줄인다**:
