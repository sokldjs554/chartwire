...
# 홈(콘솔이 스스로 clinician 으로 로그인) → 새 상담(--patient, --script auto = 카탈로그에서 예상 경보가
#   있는 첫 대본; 녹음 길이는 그 대본의 total_ms) → 실시간 기록 → 위험 배너 → ACK → 종료
#   → 초안·근거(note.status 가 오면 콘솔이 자동으로 불러온다) → 데이터 관리(동의 철회 → 파기 → 영수증
#   → 복호화 시도; 영수증 조회는 admin 전용이라 콘솔이 스스로 역할을 바꾼다) → 시스템 상세(Ops)
# 스크린샷: docs/images/00_intro.png … 05_ops.png
```

콘솔이 보여 주는 것(§13.3 위에 얹은 안내 층):

- **앱 셸** — 왼쪽 사이드바(홈 · 새 상담 · 상담 기록 · 검토 대기 · 데이터 관리 · 시스템 상세)와 상단 바 한 벌, 그 안에서 `.page` 가
  바뀐다. 720 px 이하에서는 사이드바가 가로 스트립으로 눕는다 — 숨기면 '데이터 관리'(파기 영수증)에 갈 방법이 없어진다.
- **실제/미리보기 판정** — 부팅 때 `GET /v1/release` 를 한 번 찔러 200 이면 자기 오리진의 실제 API 를 쓰고, 실패할 때만 미리보기로
  내려간다(정적 호스팅처럼 백엔드가 아예 없는 경우). 호스트 이름으로 정하면 `docker compose` 와 localhost 가 조용히 목업이 된다.
  미리보기에서도 **파기 증적만은 지어내지 않는다**: `/v1/purge-jobs` 는 거절하고 영수증 자리를 「발급 안 됨」으로 비운다.
- **홈** — 히어로 · 상담 업무 흐름 4단계 · 핵심 기능 4장 · 검증된 범위 3칸(숫자 마커) · 최근 상담. 
- **새 상담** — 합성 대본을 고르고 「합성 상담 실행」을 누르면 가상환자·동의·세션을 만들고 WebSocket 기록을 시작한다. 오른쪽에
  근거가 붙은 초안이 뜨고, 문장을 누르면 그 문장이 인용한 원문이 펼쳐진다.
- **검토 대기** — 초안이 있을 수 있는 세션만 묻는다(`created`·`purged` 는 건너뛴다 — 전부 물으면 `notes/latest` 404 가 세션 수만큼 쌓여
  "브라우저 오류 0" 게이트가 정상 동작에서도 실패한다).
- **데이터 관리** — 수명주기 5단계 · 동의 철회 → 파기 → 영수증 → 복호화 시도. 영수증 조회와 verify-decrypt 는 admin·auditor 전용이라
  콘솔이 스스로 역할을 바꿔 다시 로그인한다.
- **시스템 상세** — 프로토콜 · RLS · outbox · crypto-shred 카드(측정치는 숫자 마커)와 `/metrics` 를 읽는 운영 지표.
...
- GIF 만들기(10 fps, 폭 940, 팔레트 40색, ≤ 8 MB — 18 초 녹화가 3.5 MB; 따뜻한 팔레트로 바꾼 뒤 그라디언트가 색을 더 먹어 1100 → 1000 → 940 으로 두 번 좁혔다). 8 MB 를 넘으면 **색을 줄이기 전에 폭을 줄인다**:
