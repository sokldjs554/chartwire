...
- 구절은 **어간(stem)** 입니다. `죽고 싶` 은 `죽고 싶어요 / 죽고 싶다는 / 죽고 싶은 마음` 을 모두 잡고, 어미가 뒤에 붙는 방언(`죽고 싶데이`, `살기 싫어예`, `죽고 싶당께`, `죽고 싶어유`)도 같은 어간으로 걸립니다.
- **매칭은 공백을 무시합니다** (`scope.find_spans`): `죽고싶어요`·`죽고  싶어요`·`손목그어요` 가 모두 어간에 걸리므로 띄어쓰기 변형마다 항목을 추가하지 않습니다. 관용구 목록도 같은 방식으로 비교합니다.
- 표기 변형(`죽구 싶`, `뒤지고 싶`, `사라지구 싶`)·간접 표현(`살아서 뭐하나`, `눈 안 떠졌으면`, `세상 뜨고 싶`, `극단적인 선택`)·수단(`약 모아`, `옥상에 올라`, `한강 다리`, `수면제 한꺼번에`)·자해(`상처 내`, `머리 박`, `자해 충동`, `꼬집`, `담뱃불`)·타해(`죽여버리`, `때려죽이`, `불 지르`, `다 죽여`)·급성 물질(`필름 끊`, `블랙아웃`, `며칠째 술`, `마시고 운전`, `약 과다`)을 포함합니다.
- 범주 4종: `suicidal_ideation`, `self_harm`, `harm_to_others`, `substance_acute` (`risk_events.category` CHECK와 동일).
- 등급 3단계:

| severity | 의미 | 예 | SLA (§7.4) |
|---|---|---|---|
| 3 | 계획·수단·시도 | `약을 모아`, `뛰어내리`, `목을 매`, `유서`, `번개탄`, `손목을 긋`, `칼로 그`, `술을 마시고 약을` | 60 s |
...
| `past` | 문장에 과거 표지(`예전 / 작년 / 그때 / 했었 / 었는데 / 전에는 …`) | 단독이면 severity −1, 결과가 ≥1이면 경보 |
| `present_denial` | `past` 이면서 히트 뒤에 현재 표지(`지금은 / 요즘은 / 이제는 …`) **그리고** 그 뒤에 부정 | **억제**. 현재절의 부정은 `negated`로 **다시 세지 않음** |
...
**억제 = `negated ∨ hypothetical ∨ third_person ∨ clinician_question ∨ idiom ∨ (past ∧ present_denial)`.**

**과거 사고(思考)의 해석**: 히트가 과거 표지(`예전 / 작년 / 그때 / 했었 / 었는데 …`)와 함께 나타나고 **현재 부정**(`지금은 아니 / 요즘은 없 / 이제는 안 …`)이 뒤따르면 **억제**하고, 현재 부정이 없으면 **severity 를 1 낮춰 경보**합니다. (품질 패스 2; `docs/eval/README.md` 해석 규칙 1 과 같은 문장)
 경보 = `severity ≥ 1 ∧ ¬억제` (`RiskHit.alerts`). 임상가의 **평서문**(`환자분이 약을 모아두셨다고요`)은 억제되지 않습니다 — 안전망은 화자 역할을 신뢰하지 않습니다.
...
## 9. `past` 규칙 (`docs/eval/README.md` 해석 규칙 1 과 동일한 문장)
