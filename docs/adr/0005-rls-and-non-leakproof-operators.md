...
- 비용: `segment_search` 가 FORCE 가 아니므로 owner 로 접속한 도구(마이그레이션·벌크 로더)는 정책을 보지
  않는다 — 런타임은 owner 로 접속하지 않는다는 ADR-0001 §0.7 이 **전제**다. 그 전제가 깨지는 곳은
  관리형 PG 단일 역할 폴백(런타임 = 소유자)뿐이고, 거기서는 0006 이 `app_role_exists()` 가 False 인 것을
  보고 `segment_search` 에도 FORCE 를 걸어 격리를 지킨다 — 대신 면제가 사라져 검색이 Q2b 경로(느림)로
  떨어진다. 즉 빠른 경로는 런타임이 소유자가 **아닌** 배포에서만 성립한다.
